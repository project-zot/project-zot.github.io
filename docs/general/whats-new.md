...
- The two types of state (images and image metadata) can both now be managed on [remote storage](https://github.com/project-zot/zot/blob/v1.4.3/examples/config-all-remote.json) so that `zot` process lifecycle and its storage can be managed and scaled independently.
